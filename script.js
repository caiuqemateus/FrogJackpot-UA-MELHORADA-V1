document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // Removemos o clearSelection() daqui
    ...
    quickBetButton.addEventListener('click', async () => {
        ...
        try {
            // Seleciona 6 sapos aleatoriamente (permite repetições)
            for (let i = 0; i < gameState.maxSelections; i++) {
                const randomIndex = Math.floor(Math.random() * frogCards.length);
                const randomFrog = frogCards[randomIndex];
